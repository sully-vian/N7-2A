...
let order i1 i2 =
  match i1, i2 with
  | Infty_neg, _ | _, Infty_pos -> true
  | Fin i, Fin j -> Int64.compare i j <= 0
  | Infty_pos, _ | _, Infty_neg -> false
;;
...
let to_int = function
  | Fin x
    when Int64.compare x (Int64.of_int min_int) >= 0
...
let add_lt_min_int x y =
  Int64.compare x Int64.zero < 0
  && Int64.compare y Int64.zero < 0
...
let add_lb x y =
  ...
  | Fin _, Infty_pos | Infty_pos, Fin _ | Infty_pos, Infty_pos -> Infty_pos
  | _ -> Infty_neg
;;
...
let add_ub x y =
  ...
  | Fin _, Infty_neg | Infty_neg, Fin _ | Infty_neg, Infty_neg -> Infty_neg
  | _ -> Infty_pos
;;
...
let sub_lb x y =
  ...
  | Fin _, Infty_neg | Infty_pos, Fin _ | Infty_pos, Infty_neg -> Infty_pos
  | _ -> Infty_neg
;;
...
let sub_ub x y =
  ...
  | Fin _, Infty_pos | Infty_neg, Fin _ | Infty_neg, Infty_pos -> Infty_neg
  | _ -> Infty_pos
;;
...
let mul_lb x y =
  ...
  | (Fin x, Infty_pos | Infty_pos, Fin x) when Int64.compare x Int64.zero > 0 -> Infty_pos
  | (Fin x, Infty_neg | Infty_neg, Fin x) when Int64.compare x Int64.zero < 0 -> Infty_pos
  | _ -> Infty_neg
;;
...
let mul_ub x y =
  ...
  | (Fin x, Infty_neg | Infty_neg, Fin x) when Int64.compare x Int64.zero > 0 -> Infty_neg
  | (Fin x, Infty_pos | Infty_pos, Fin x) when Int64.compare x Int64.zero < 0 -> Infty_neg
  | _ -> Infty_pos
;;
...
let div_lb x y =
  match x, y with
  | Fin x, Fin y ->
    (try Fin (Int64.div x y) with
     | Division_by_zero -> Infty_neg)
  | Fin x, Infty_pos | Fin x, Infty_neg -> zero
