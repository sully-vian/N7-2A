...
module Wrapper (T : TypeAndManager) : Relational.Domain = struct
  ...
  let widening x y =
    let y = join x y in
    (* Apron's widening requires that its first argument
  ...
  let rec apron_texpr_of_expr env e =
    let rec apron_expr_of_expr = function
      ...
      | Ast.Rand (_, n1, n2) -> Apron.Texpr1.Cst (Apron.Coeff.i_of_int n1 n2)
    in
  ...
  let assignment n e t =
    ...
    Apron.Abstract1.assign_texpr_array manager t [| Apron.Var.of_string n |] [| e |] None
  ;;
