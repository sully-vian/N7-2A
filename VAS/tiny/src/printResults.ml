...
module Make (Dom : Relational.Domain) = struct
  let alarms m t =
    let top, bottom =
      ...
      Dom.top vars, Dom.bottom vars
    in
    let find_or_top l =
      try Location.Map.find l m with
      | Not_found -> top
    in
    ...
    let rec alarms_stm env s =
      let alarms_env_beg_stm s =
        fst (alarms_stm (find_or_top (Location.beg_p (Ast.loc_of_stm s))) s)
      in
      if Dom.order env bottom then (
  ...
  let print m t output_filename =
    ...
    let rec fprint_stm ff s =
      let fprint_annot f ff l =
        try
          ...
          if Report.silent (fun () -> Dom.order top a) then
            ...
            Format.fprintf ff f Dom.fprint a
        with
        | Not_found -> ()
      in
      ...
      | Ast.Ite (l, e, s1, s2) ->
        Format.fprintf
          ff
          "@[<v>@[<v 2>if (@[%a@] > 0) {%a@ %a@]@ @[<v 2>} else {%a@ %a@]@ }%a@]"
          Ast.fprint_expr
          e
          fprint_annot
          (Location.beg_p (Ast.loc_of_stm s1))
          fprint_stm
          s1
          fprint_annot
          (Location.beg_p (Ast.loc_of_stm s2))
          fprint_stm
          s2
          fprint_annot
          (Location.end_p l)
      | Ast.While (l, e, s) ->
        Format.fprintf
          ff
          "@[<v>%a@[<v 2>while (@[%a@] > 0) {%a@ %a@]@ }%a@]"
          fprint_invariant
          (Location.beg_p (Ast.loc_of_expr e))
          Ast.fprint_expr
          e
          fprint_annot
          (Location.beg_p (Ast.loc_of_stm s))
          fprint_stm
          s
          fprint_annot
          (Location.end_p l)
    ...
    Utils.with_out_ch output_filename (fun out_ch ->
      ...
      fprint_stm ff t;
      Format.fprintf ff "\n%!");
    Report.nlogf 1 "Results written to %s." (Utils.output_filename_string output_filename)
  ;;
end
