...
module MakeRelational (D : Domain) : Relational.Domain = struct
  ...
  let find_or_top n m =
    try Name.Map.find n m with
    | Not_found -> D.top
  ;;

  let order_print n x y =
    let b = D.order x y in
    Report.nlogf n "%a@ ⊑ %a@ = %b." D.fprint x D.fprint y b;
    b
  ;;
  ...
  let order t1 t2 =
    ...
    | _, EnvBot -> false
    | Env m1, Env m2 ->
      Name.Map.fold (fun n v2 b -> b && order_print 5 (find_or_top n m1) v2) m2 true
  ;;

  let top _ = Env Name.Map.empty
  let bottom _ = EnvBot

  let join_f s f t1 t2 =
    match t1, t2 with
    | EnvBot, _ -> t2
    | _, EnvBot -> t1
    | Env m1, Env m2 ->
      let m =
        Name.Map.fold
          (fun n v1 m ->
             ...
             let v = f v1 v2 in
             Report.nlogf 5 "%a@ %s %a@ = %a." D.fprint v1 s D.fprint v2 D.fprint v;
             if order_print 6 D.top v then
               m
             else
               Name.Map.add n v m)
          m1
  ...
  let meet_print x y =
    let r = D.meet x y in
    Report.nlogf 5 "%a@ ⊓ %a@ = %a." D.fprint x D.fprint y D.fprint r;
    r
  ;;
         ...
         let m =
           Name.Map.fold
             (fun n v1 m ->
                ...
                if order_print 6 v D.bottom then
                  raise Exit
                else
                  Name.Map.add n v m)
             m1
  ...
  let rec eval_expr env = function
    ...
    | Ast.Binop (_, bop, e1, e2) ->
      ...
      in
      let v = sem v1 v2 in
      Report.nlogf 5 "%s@ %a@ %a@ = %a." s D.fprint v1 D.fprint v2 D.fprint v;
      v
    | Ast.Rand (_, n1, n2) ->
  ...
  let assignment n e t =
    ...
    | Env m ->
      ...
      if order_print 6 t D.bottom then
        EnvBot
      else if order_print 6 D.top t then
        Env (Name.Map.remove n m)
      else
        Env (Name.Map.add n t m)
  ;;
  ...
  let guard e t =
    ...
    | Env m ->
      ...
      (try Env (backeval_expr m e t') with
       | Exit -> EnvBot)
  ;;
end
