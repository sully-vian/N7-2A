...
let rec retrait_arbre lc (Noeud (b, lb)) =
  ...
  | c :: qlc ->
      let arbre_c =
        let l = recherche c lb in
        match l with None -> Noeud (b, lb) | Some a -> a
      in
      Noeud (b, maj c (retrait_arbre qlc arbre_c) lb)
...
let rec parcours (Noeud (b, lb)) =
  ...
  let liste =
    List.flatten
      (List.map
         (fun (c, arbre_c) -> List.map (fun l -> c :: l) (parcours arbre_c))
         lb)
    (* si b, alors on est à la fin d'un mot, on rajoute une "fin de mot" vide avec [] *)
  in
  if b then [] :: liste else liste
...
let rec normaliser2 (Noeud (b, lb)) = Noeud (b, parcours_branches lb)

and parcours_branches branches =
  match branches with
  | [] -> []
  | (c, t) :: q -> (
      match normaliser2 t with
