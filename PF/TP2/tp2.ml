...
*)
let listStat =
  let input = open_in "/home/vhy9665/Downloads/nat2016.txt" in
...
*)
let listStatHomme =
  let input = open_in "/home/vhy9665/Downloads/nathomme2016.txt" in
  let filebuf = Lexing.from_channel input in
  Parser.main Lexer.token filebuf
...
let listStatTriee2 = List.sort (fun x y -> if (no4 x > no4 y) then 1 else -1) listStat
