...
module Parser : Parsing = struct
  ...
  let ( >>= ) parse dep_parse f =
    Solution.(parse f >>= fun (b, f') -> dep_parse b f')
...
and cmd =
  | Repeat of
      int * prog (* on repete n fois un programme                      *)
...
let p_chaine s =
  let rec parse i =
    if i < 0 then return () else map fst (parse (i - 1) *> p_car s.[i])
  in
...
let rec parse_P : (char, prog) parser =
 fun flux ->
  print_endline "Parse_P appelé";
  ( p_begin >>= fun _ ->
    parse_I >>= fun instruction ->
    p_end >>= fun _ -> return instruction )
    flux

and parse_I : (char, inst) parser =
 fun flux ->
  print_endline "Parse_I appelé";
  ((return [])
  ++ ( parse_C >>= fun commande ->
       p_ptvirg >>= fun _ ->
       parse_I >>= fun instruction -> return (commande :: instruction) ))
    flux

and parse_C : (char, cmd) parser =
 fun flux ->
  print_endline "Parse_C appelé";
  (( p_repeat >>= fun _ ->
     p_entier >>= fun entier ->
     parse_P >>= fun programme -> return (Repeat (entier, programme)) )
  ++ ( p_move >>= fun _ ->
       p_entier >>= fun entier -> return (Move entier) )
  ++ ( p_turn >>= fun _ ->
       p_entier >>= fun entier -> return (Turn entier) )
  ++ (p_on >>= fun _ -> return On)
  ++ (p_off >>= fun _ -> return Off))
    flux

(* fonction principale de parsing des programmes LOGO *)
let parse_logo flux = print_endline "parse_logo appelé";run (map fst (parse_P *> p_eof)) flux
...
let rec logo_to_string p = String.concat "; " (List.map cmd_to_string p)
...
and exec_cmd (on, x, y, a) cmd =
  ...
  | Repeat (n, p) ->
      ...
      else exec_cmd (exec_logo (on, x, y, a) p) (Repeat (n - 1, p))
  | Move d ->
      let x' = x +. (float_of_int d *. cos (rad_of_deg *. a))
      and y' = y +. (float_of_int d *. sin (rad_of_deg *. a)) in
      (if on then Graphics.lineto else Graphics.moveto)
