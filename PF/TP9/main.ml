...
(* les parsers mutuellement récursifs pour la grammaire ci-dessus: DONE *)
let rec parse_P : (char, prog) parser =
  fun flux ->
  (p_begin
   >>= fun _ -> parse_I >>= fun instruction -> p_end >>= fun _ -> return instruction)
    flux
