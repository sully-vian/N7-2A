...
let rec string_of_ast tree =
  ...
  | IfthenelseNode (cond, bthen, belse) ->
    ...
    ^ string_of_ast belse
    ^ ")"
  | FunctionNode (par, body) -> "(fun " ^ par ^ " -> " ^ string_of_ast body ^ ")"
  | CallNode (func, par) -> "((" ^ string_of_ast func ^ ") " ^ string_of_ast par ^ ")"
  ...
  | WriteNode (id, exp) -> "(" ^ string_of_ast id ^ " := " ^ string_of_ast exp ^ ")"
