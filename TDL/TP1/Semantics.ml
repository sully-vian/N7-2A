...
(* Expression non définie à traiter par les étudiants *)
...
and
  ...
  let leftvalue = value_of_expr left env (* Evaluation du paramètre gauche *) in
...
and
  ...
  match value_of_expr _bvalue _env with
  | ErrorValue _ as result -> result
  | result ->
      let _env' = (_ident, result) :: _env in
...
and
  ...
  match value_of_expr _cond _env with
  | ErrorValue _ as result -> result
