...
type typeType =
  ...
  | UnknownType
  (* Type inconnu utilisé pour les *)
  (* variables de type avant unification *)
  | VariableType of (typeType ref * int)
  (* Variable de type identifiée par un numéro *)
  (* et contenant une référence sur un type permettant *)
  (* l'unification *)
  | FunctionType of
      typeType * typeType (* Type fonction d'un paramètre vers un résultat *)
  | UnitType (* Type vide *)
...
(** [newVariable ()] crée une variable de type, différente des précédentes dont
    la valeur initiale (modifiable) est Unknown *)
let newVariable () =
  counter := !counter + 1;
...
and string_of_type_env env =
  ...
  | [] -> ""
  | (key, typevalue) :: q ->
      ("{" ^ key ^ " = " ^ string_of_type typevalue)
      ^ "} " ^ string_of_type_env q

(** [normalize t] normalise le type [t] en remplaçant les variables de type par
    leur valeur si elles sont connues, et en propagant l'erreur si elle est
    présente dans un type structuré. *)
let rec normalize t =
  match t with
  | (IntegerType | BooleanType | UnitType | ErrorType | UnknownType) as result
    ->
      result
  ...
  | ReferenceType t ->
      ...
      if nt <> ErrorType then
        ReferenceType nt
      else
        ErrorType

(** [unify t1 t2] compare les types [t1] et [t2] (en évaluant éventuellement les
    variables de type présentes dans chacun des deux). La valeur renvoyée est le
    premier type et [true] s'il y a correspondance, [ErrorType] et [false]
    sinon. *)
let unify t1 t2 =
  (* test de presence de v dans t : pour eviter une boucle infinie dans
     l'unification *)
  let rec occur_check v t =
    if !debug then
      print_endline ("OC : " ^ string_of_int v ^ " in " ^ string_of_type t);
    match t with
    (* Une variable ne peut pas appartenir à un type constant *)
    | IntegerType | BooleanType | UnitType | ErrorType | UnknownType -> false
    (* Une variable peut appartenir au type du paramètre ou au type du
       résultat *)
  ...
  and
      ...
      aux t1 t2 =
    match (t1, t2) with
    (* Si les 2 types sont identiques, l'unification réussit et renvoie le même
       type *)
    | IntegerType, IntegerType -> (IntegerType, true)
    | BooleanType, BooleanType -> (BooleanType, true)
    | UnitType, UnitType -> (UnitType, true)
    (* L'unification de 2 types pointeurs réussit si l'unification des types
       pointés réussit *)
    ...
    (* L'unification de 2 variables de type réussit s'il s'agit de la même
       variable *)
    ...
    | VariableType (v1, _), VariableType (v2, _)
      when !v1 <> UnknownType && !v2 <> UnknownType ->
        aux !v1 !v2
    ...
    | _, VariableType (v2, r2) when !v2 = UnknownType && not (occur_check r2 t1)
      ->
        v2 := t1;
        (t1, true)
  ...
  if !debug then print_endline ("result " ^ string_of_type t);
...
and ruleBoolean = BooleanType
and ruleInteger = IntegerType

and ruleAccess env name =
  match lookforEnv name env with NotFound -> ErrorType | Found t -> t

and ruleUnary env op expr =
...
and ruleLet env ident bvalue bin =
  let tident = type_of_expr bvalue env in
  let newenv = (ident, tident) :: env in
  type_of_expr bin newenv

and ruleIf env cond ethen eelse =
...
and ruleFunction env par body =
  let tpar = newVariable () in
  let newenv = (par, tpar) :: env in
  let tbody = type_of_expr body newenv in
  if tpar = ErrorType || tbody = ErrorType then
    ErrorType
  else
    FunctionType (tpar, tbody)

and ruleCall env fct arg =
  let tfct = type_of_expr fct env in
  let targ = type_of_expr arg env in
  match tfct with
  | VariableType _ ->
      (* cas d'une fonction récursive, ne se connaît pas encore *)
      let tret = newVariable () in
      let expected = FunctionType (targ, tret) in
      let _, ur = unify tfct expected in
      if ur then
        tret
      else
        ErrorType
  | FunctionType (tpar, tret) ->
      let _, ur = unify targ tpar in
      if ur then
        tret
      else
        ErrorType
...
and ruleLetrec env ident bvalue bin =
  let tvar = newVariable () in
  let newenv = (ident, tvar) :: env in
  let tident = type_of_expr bvalue newenv in
  let _, ur = unify tvar tident in
  if ur then
    type_of_expr bin newenv
  else
    ErrorType

and ruleUnit = UnitType

and ruleRef env expr =
  let texpr = type_of_expr expr env in
  ReferenceType texpr

and ruleRead env expr =
  let texpr = type_of_expr expr env in
  match texpr with ReferenceType t -> t | _ -> ErrorType

and ruleWrite env left right =
  let tleft = type_of_expr left env in
  let tright = type_of_expr right env in
  let _, ur = unify tleft (ReferenceType tright) in
  if ur then
    UnitType
  else
    ErrorType

and ruleSequence env left right =
  let tleft = type_of_expr left env in
  let tright = type_of_expr right env in
  let _, ur = unify tleft UnitType in
  if ur then
    tright
  else
    ErrorType

and ruleWhile env cond body =
  let tcond = type_of_expr cond env in
  let tbody = type_of_expr body env in
  let _, urc = unify tcond BooleanType in
  let _, urb = unify tbody UnitType in
  if urc && urb then
    UnitType
  else
    ErrorType
