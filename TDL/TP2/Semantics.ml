...
(* Converti une liste de chaînes de caractères en une seule chaîne de
   caractères *)
let string_of_names names = List.fold_right (fun t tq -> t ^ " " ^ tq) names ""

(* string_of_env : environment -> string *)
(* Convertit un environnement en une chaine de caractères en vue de son
   affichage *)
let rec string_of_env env =
  match env with
  | [] -> ""
  | (key, value) :: q ->
      (key ^ "," ^ string_of_value value) ^ ";" ^ string_of_env q
...
and string_of_mem mem =
  ...
  | [] -> ""
  | (key, value) :: q ->
      (key ^ "," ^ string_of_value value) ^ ";" ^ string_of_mem q

(* string_of_value : valueType -> string *)
(* Convertit une valueType en une chaine de caractères en vue de son
   affichage *)
...
and string_of_value_and_mem vm =
  "Valeur = "
  ^ string_of_value (fst vm)
  ^ "\nMemoire = "
  ^ string_of_mem (snd vm)
...
(* Cherche un identifiant dans un environnement et renvoie la valeur associée à
   cet identifiant ou une erreur le cas échéant *)
let rec lookforEnv name env =
  ...
  | (key, value) :: others ->
      if key = name then
        ...
        lookforEnv name others

(* lookfor : string -> memory -> valueType searchResult *)
(* Cherche une adresse dans une mémoire et renvoie la valeur associée à cette
   adresse ou une erreur le cas échéant *)
...
and ruleLet env ident bvalue bin mem =
  let vval, vmem = value_of_expr (bvalue, mem) env in
  match vval with
  | ErrorValue _ as result -> (result, vmem)
  | _ -> value_of_expr (bin, vmem) ((ident, vval) :: env)

and ruleBinary env op left right mem =
  let rightvalue, rightmem = value_of_expr (right, mem) env in
...
and ruleUnary env op expr mem =
  let rvalue, rmem = value_of_expr (expr, mem) env in
  match rvalue with
  | ErrorValue _ as result -> (result, rmem)
  | IntegerValue value -> (
      match op with
      | Opposite -> (IntegerValue (-value), rmem)
      | _ -> (ErrorValue TypeMismatchError, rmem))
  | BooleanValue value -> (
      match op with
      | Negation -> (BooleanValue (not value), rmem)
      | _ ->
          (ErrorValue TypeMismatchError, rmem)
          (*| _ -> ((ErrorValue TypeMismatchError),rmem) *))
  | _ -> (ErrorValue TypeMismatchError, rmem)

and ruleIf env cond bthen belse mem =
  let cval, cmem = value_of_expr (cond, mem) env in
  match cval with
  | BooleanValue rcond ->
      if rcond then
...
(* Appel par nom and (*
   ...
   TypeMismatchError, mem2) result | _ -> (ErrorValue TypeMismatchError) *)

and ruleCallByValue env fexpr pexpr mem =
  let pval, pmem = value_of_expr (pexpr, mem) env in
  match pval with
  | ErrorValue _ as result -> (result, pmem)
  | _ -> (
      let fval, fmem = value_of_expr (fexpr, pmem) env in
      match fval with
...
and ruleLetrec env ident bvalue bin mem =
  value_of_expr (bin, mem)
    ((ident, FrozenValue (LetrecNode (ident, bvalue, bvalue), env)) :: env)

and ruleTrue mem = (BooleanValue true, mem)
and ruleFalse mem = (BooleanValue false, mem)
and ruleUnit mem = (NullValue, mem)
and ruleInteger value mem = (IntegerValue value, mem)

and ruleRead _env _expr mem =
  let pval, mem2 = value_of_expr (_expr, mem) _env in
  match pval with
  | ReferenceValue add -> (
      match lookforMem add mem2 with
      | NotFound -> (ErrorValue (UnknownReferenceError add), mem2)
      | Found v -> (v, mem2))
  | _ -> (ErrorValue TypeMismatchError, mem2)

and ruleWrite : environment -> ast -> Ast.ast -> memory -> valueType * memory =
 fun _env _refexpr _valexpr mem ->
  let pval, mem2 = value_of_expr (_valexpr, mem) _env in
  match pval with
  | ErrorValue _ as result -> (result, mem2)
  | _ -> (
      let pvalref, mem3 = value_of_expr (_refexpr, mem2) _env in
      match pvalref with
      | ReferenceValue add -> (
          match lookforMem add mem3 with
          | NotFound -> (ErrorValue (UnknownReferenceError add), mem3)
          | Found _ -> (NullValue, (add, pval) :: mem3))
      | _ -> (ErrorValue TypeMismatchError, mem3))

and ruleSequence :
    environment -> Ast.ast -> Ast.ast -> memory -> valueType * memory =
 fun _env _left _right mem ->
  let lval, mem2 = value_of_expr (_left, mem) _env in
  match lval with
  | NullValue ->
      let rval, mem3 = value_of_expr (_right, mem2) _env in
      (rval, mem3)
  | ErrorValue _ as result -> (result, mem2)
  | _ -> (ErrorValue TypeMismatchError, mem2)
...
and ruleWhile :
    environment -> Ast.ast -> Ast.ast -> memory -> valueType * memory =
 fun _env _cond _body mem ->
  let pval, mem2 = value_of_expr (_cond, mem) _env in
  match pval with
  | ErrorValue _ as result -> (result, mem2)
  | BooleanValue true -> (
      let pval2, mem3 = value_of_expr (_body, mem2) _env in
      match pval2 with
      | ErrorValue _ as result -> (result, mem3)
      | NullValue -> value_of_expr (WhileNode (_cond, _body), mem3) _env
      | _ -> (ErrorValue TypeMismatchError, mem3))
  | BooleanValue false -> (NullValue, mem2)
  | _ -> (ErrorValue TypeMismatchError, mem2)

and ruleReference : environment -> Ast.ast -> memory -> valueType * memory =
 fun _env _expr mem ->
  let pval, mem2 = value_of_expr (_expr, mem) _env in
  match pval with
  | ErrorValue _ as result -> (result, mem2)
  | result -> (
      ...
      match lookforMem add mem2 with
      | Found _ -> (ErrorValue RuntimeError, mem2)
      | NotFound -> (ReferenceValue add, (add, result) :: mem2))
