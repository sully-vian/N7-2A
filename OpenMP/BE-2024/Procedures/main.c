...
int main(int argc, char **argv) {
    ...
    if (argc == 2) {
        ...
        printf("Usage:\n\n ./main np\n\nwhere np is the number of procedures "
               "to execute.\n");
        return 1;
    }
    init(np);
    ...
    t_start = usecs();
    // parallelize the execution of the procedures
    // private(procedure): each thread needs its own procedure
    // private(s): each thread counts its own steps
#pragma omp parallel for private(procedure, s)
    for (p = 0; p < np; p++) {

        // get_procedure is not thread-safe
#pragma omp critical
