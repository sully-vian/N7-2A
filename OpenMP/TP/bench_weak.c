...
NB is the number of block-columns the matrix is made of.\n");
  ...
  fptr = fopen("weak_scalability.csv", "w");

  for (i = 1; i <= maxloop; i *= 2) {
    nth = MIN(i, maxnth);
    omp_set_num_threads(nth);
    iNB = ceil(NB * (cbrt((double)nth)));
    N = B * iNB;
    NN = N * N;
    ...
    /* printf("||Ax-b||     : %.4e\n",check_res(A, Acpy)); */
    printf("%7.1f,   %7.1f,   ", (t_end - t_start) / 1e3,
           flops / (t_end - t_start) / 1e3 / ((double)nth));
    fprintf(fptr, "%7.1f,   %7.1f,   ", (t_end - t_start) / 1e3,
            flops / (t_end - t_start) / 1e3 / ((double)nth));
