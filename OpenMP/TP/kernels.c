...
void trsm(block_t akk, block_t aik) {
  char NoTran = 'N', Lower = 'L', Unit = 'U', Left = 'L', Right = 'R',
       Tran = 'T', NoUnit = 'N';
  int info;
  ...
  printf("%2d --  trsm on (%2d,%2d) using (%2d,%2d)\n", omp_get_thread_num(),
         aik.r, aik.c, akk.r, akk.c);
#endif
  trace_event_start(TRSM);
#if defined(SIM)
  mysleep(pow((double)aik.B, 3) / PEAK);
#else
  dtrsm_(&Right, &Lower, &Tran, &NoUnit, &aik.B, &aik.B, &DONE, akk.a, &akk.B,
         aik.a, &aik.B);
#endif
...
void gemm(block_t aik, block_t ajk, block_t aij) {
  ...
  if (aik.r == ajk.r) {
    ...
    mysleep(pow((double)aik.B, 3) / PEAK);
#else
    dsyrk_(&Lower, &NoTran, &aij.B, &aik.B, &DMONE, aik.a, &aik.B, &DONE, aij.a,
           &aij.B);
#endif
  } else {
#if defined(SIM)
...
#else
    dgemm_(&NoTran, &Tran, &aij.B, &aij.B, &aik.B, &DMONE, aik.a, &aik.B, ajk.a,
           &ajk.B, &DONE, aij.a, &aij.B);
#endif
  }
...
double check_res(matrix_t A, matrix_t Ac) {
  ...
  for (i = 0; i < A.N; i++) x[i] = b[i];

  for (j = 0; j < A.NB; j++) {
    dtrsm_(&Left, &Lower, &NoTran, &NoUnit, &A.blocks[j][j].B, &IONE, &DONE,
           A.blocks[j][j].a, &A.blocks[j][j].B, x + j * A.B, &A.N);
    for (i = j + 1; i < A.NB; i++) {
      dgemm_(&NoTran, &NoTran, &A.blocks[i][j].B, &IONE, &A.blocks[i][j].B,
             &DMONE, A.blocks[i][j].a, &A.blocks[i][j].B, x + j * A.B, &A.N,
             &DONE, x + i * A.B, &A.N);
    }
  }

  for (j = A.NB - 1; j >= 0; j--) {
    dtrsm_(&Left, &Lower, &Tran, &NoUnit, &A.blocks[j][j].B, &IONE, &DONE,
           A.blocks[j][j].a, &A.blocks[j][j].B, x + j * A.B, &A.N);
    for (i = j - 1; i >= 0; i--) {
      dgemm_(&Tran, &NoTran, &A.blocks[j][i].B, &IONE, &A.blocks[j][i].B,
             &DMONE, A.blocks[j][i].a, &A.blocks[j][i].B, x + j * A.B, &A.N,
             &DONE, x + i * A.B, &A.N);
    }
  }
  ...
  for (j = 0; j < Ac.NB; j++) {
    nrm =
        dnrm2_c(Ac.blocks[j][j].B * Ac.blocks[j][j].B, Ac.blocks[j][j].a, IONE);
    norma += nrm + nrm;
    dsymm_(&Left, &Lower, &Ac.blocks[j][j].B, &IONE, &DMONE, Ac.blocks[j][j].a,
           &Ac.blocks[j][j].B, x + j * Ac.B, &Ac.N, &DONE, b + j * Ac.B, &Ac.N);
    for (i = j + 1; i < Ac.NB; i++) {
      nrm = dnrm2_c(Ac.blocks[i][j].B * Ac.blocks[i][j].B, Ac.blocks[i][j].a,
                    IONE);
      norma += 2 * nrm + nrm;
      dgemm_(&NoTran, &NoTran, &Ac.blocks[i][j].B, &IONE, &Ac.blocks[i][j].B,
             &DMONE, Ac.blocks[i][j].a, &Ac.blocks[i][j].B, x + j * Ac.B, &Ac.N,
             &DONE, b + i * Ac.B, &Ac.N);
      dgemm_(&Tran, &NoTran, &Ac.blocks[i][j].B, &IONE, &Ac.blocks[i][j].B,
             &DMONE, Ac.blocks[i][j].a, &Ac.blocks[i][j].B, x + i * Ac.B, &Ac.N,
             &DONE, b + j * Ac.B, &Ac.N);
    }
  }
