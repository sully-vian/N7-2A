...
#pragma omp master
    {
        ...
        for (i = 0; i < MAXTHREADS; i++) nevents[i] = 0;
...
void trace_dump(char *fname) {
    ...
    Event ev;
    FILE *pFile;
    char *colors[] = {"#77ac30", "#a2142f", "#0072bd", "#80b3ff", "#e580ff"};
...
#pragma omp master
    {
        ...
        pFile = fopen(fname, "w");
        fprintf(pFile, "<svg xmlns=\"http://www.w3.org/2000/svg\" x=\"-11\" y=\"-8\" width=\"%f\" height=\"%f\">\n",
                ((double)t_stop - t_zero) * 1.06 / scale_x,
                ((double)nth + 1) * 1.05 / scale_y + 8);
        ...
        for (t = 0; t < MAXTHREADS; t++) {
            for (e = 0; e < nevents[t]; e++) {
                ...
                fprintf(pFile,
                        ...
                        ((double)1.0) / scale_y, colors[ev.type]);
            }
        }

        fprintf(pFile,
                "<line x1=\"0\" y1=\"%f\" x2=\"%f\" y2=\"%f\" \
        ...
        for (t = 0; t < nth; t++) {
            fprintf(pFile,
                    "<text x=\"%f\" y=\"%f\" style=\"font-size:6pt\">Thread "
                    "%d</text>\n",
                    ((double)t_stop - t_zero) * 1.01 / scale_x,
                    ((double)t + 0.8) / scale_y, t);
        }

        fprintf(pFile,
                "<text x=\"0\" y=\"%f\" style=\"font-size:6pt\">Time "
                "(usec.)</text>\n",
                ((double)nth + 1) * 1.05 / scale_y + 7);

        for (x = ((double)t_stop - t_zero) / scale_x / 10.0;
             x <= ((double)t_stop - t_zero) / scale_x;
             x += ((double)t_stop - t_zero) / scale_x / 10.0) {
            fprintf(pFile,
                    "<line x1=\"%f\" y1=\"%f\" x2=\"%f\" y2=\"%f\" \
                     style=\"stroke:rgb(0,0,0);stroke-width:0.5\"/>\n",
                    x, ((double)nth + 1) * 1.01 / scale_y - 1, x,
                    ((double)nth + 1) * 1.01 / scale_y + 1);
            fprintf(pFile,
                    "<text x=\"%f\" y=\"%f\" style=\"font-size:6pt\"> %.0f "
                    "</text>\n",
