...
- Visualisation du graphe des exécutions : `Model Overview` > `Additional TLC Options` (en bas à droite, parfois invisible !) > `Features` > `Visualize state graph`  
  (attention, que pour des petits graphes !)  
  Si la visualisation échoue (erreur GraphViz), aller dans `File` > `Preferences` > `TLA+ Preferences` > `PDF Viewer` > `Specify dot command`, et mettre `/usr/bin/dot`.
