...
\@x{}\moduleLeftDash\@xx{ {\MODULE} fact}\moduleRightDash\@xx{}%
\@x{ {\EXTENDS} Naturals}%
\@pvspace{8.0pt}%
\@x{ {\CONSTANT} N}%
\@x{ {\ASSUME} N \.{\in} Nat \.{\land} N \.{\geq} 1}%
\@x{ {\VARIABLES}\@s{4.1} res ,\, x}%
\@pvspace{8.0pt}%
\@x{ Init \.{\defeq} \.{\land} res \.{=} 1}%
 \@x{\@s{35.70} \.{\land} x \.{=} [ i \.{\in} 1 \.{\dotdot} N \.{\mapsto}
 {\TRUE} ]}%
\@pvspace{8.0pt}%
\@x{ OneMult ( n ) \.{\defeq} \.{\land} x [ n ]}%
 \@x{\@s{72.10} \.{\land} x \.{'} \.{=} [ x {\EXCEPT} {\bang} [ n ] \.{=}
 {\FALSE} ]}%
\@x{\@s{72.10} \.{\land} res \.{'} \.{=} res \.{*} n}%
\@pvspace{8.0pt}%
\@x{ Next \.{\defeq} \E\, n \.{\in} 1 \.{\dotdot} N \.{:} OneMult ( n )}%
\@pvspace{8.0pt}%
 \@x{ Spec\@s{1.46} \.{\defeq} Init \.{\land} {\Box} [ Next ]_{ {\langle} res
 ,\, x {\rangle}}}%
\@x{}\bottombar\@xx{}%
\end{document}
